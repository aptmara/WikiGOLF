 ...
 * @brief プレイ中のゴルフボール専用ピクセルシェーダー
 *
 * BasicPS.hlsl とほぼ同一だが、ディンプル(golfball_n.png)を小さな球体上でも
 * はっきり見せるため、ノーマルマップの傾き(XY成分)だけを強調している。
 * BasicPS.hlsl はロード画面の演出用ボールや建物など他のオブジェクトでも
 * 共有されているため、そちらには影響を与えずこのシェーダー内だけで強調する。
 */
