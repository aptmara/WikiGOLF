  ...
  for (const auto &region : regions) {
    ...
    for (int z = top; z <= bottom; ++z) {
      for (int x = left; x <= right; ++x) {
        ...
        const bool isHeading = region.kind == HtmlRegionKind::Heading;
        const bool isHazard = region.kind == HtmlRegionKind::Hazard;
        const float relief = isHeading ? 0.10f : 0.24f;
        ...
        if (!isHeading && insideRegion && playableGround) {
          materials[index] = isHazard ? 2 : 1;
        }
      }
    }
  }
