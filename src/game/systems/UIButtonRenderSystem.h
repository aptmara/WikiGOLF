...
public:
    ...
    void operator()(core::GameContext& ctx) {
        ...
        ctx.world.Query<components::UIButton>().Each([&](ecs::Entity, const components::UIButton& btn) {
            if (!btn.visible) return;

            D2D1_RECT_F rect = D2D1::RectF(btn.x, btn.y, btn.x + btn.width, btn.y + btn.height);
            ...
            D2D1_RECT_F textRect = D2D1::RectF(
                btn.x, btn.y + verticalOffset,
                btn.x + btn.width, btn.y + btn.height - verticalOffset
            );
